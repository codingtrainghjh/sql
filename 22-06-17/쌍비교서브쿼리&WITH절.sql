...
@C:\DB\db_test\sql_labs\cre_empl.sql
...
FROM employees o
WHERE salary > (SELECT AVG(salary) FROM employees i --서브쿼리 따로 실행 불가능
...
--EXISTS/NOT EXISTS 연산자 실습 --exists : 원하는 대답이 나올때까지 읽다가 답이 나오면 검색을 멈춤
